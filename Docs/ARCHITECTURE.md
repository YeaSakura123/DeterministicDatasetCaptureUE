...
`AssembleFrameGenerationDataset.py` accepts only completed source captures whose v3 validation reports pass. It verifies engine, GPU, content, shader and normalized CVar provenance; the sole intentional CVar difference is `r.MotionVectorSimulation` (`1` for endpoints, `0` for the intermediate). It verifies hashes again before copying each buffer through an atomic staging directory.

`ValidateFrameGenerationDataset.py` then checks the self-contained pair, paths, hashes, EXR shape/range/finiteness, midpoint time, camera matrices, exposure, endpoint history and provenance. A successful report is an integrity pass, not FG certification. The manifest continues to list reverse motion, UI RGBA, skeletal/WPO endpoint motion, production disocclusion and HUD-less in-world-UI validation as missing.
