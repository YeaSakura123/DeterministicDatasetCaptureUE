...
| Rigid FG endpoint motion | isolated forward/reverse processes plus `FMotionVectorSimulation` endpoint transforms | independently captured `motion_1_to_0` and `motion_0_to_1` for scene components | skeletal bones, WPO and opaque reverse event state remain outside this guarantee |
...
Frame generation uses isolated forward-endpoint, reverse-endpoint and intermediate replay roles. Its intermediate `Sτ` cannot update either endpoint's previous state. The forward process evaluates `t0 -> t1` and captures `motion_1_to_0`; the reverse process evaluates `t1 -> t0` and captures `motion_0_to_1`. Both span the full endpoint interval for rigid components, and neither field is synthesized from the other. Reverse Sequencer evaluation uses absolute jumps, so event traversal and opaque autonomous state remain adapter/cache requirements.
...
Forward endpoint replay captures `t0, t1, ...` while suppressing player Main View submissions between endpoints. Before each later endpoint render, the subsystem supplies the last captured `USceneComponent` transforms through `FMotionVectorSimulation`. Reverse endpoint replay starts at `t1`, walks toward `t0`, and supplies the saved future component transforms when rendering the earlier endpoint. The retained Main View camera history and the object previous/future transforms therefore span the same endpoint interval in independent processes. Per-frame metadata records logical evaluation direction, `motionPreviousLogicalFrameId`, the absolute time span and the limited transform scope.
...
`ValidateFrameGenerationDataset.py` then checks the self-contained pair, paths, hashes, EXR shape/range/finiteness, midpoint time, camera matrices, exposure, both endpoint histories, bidirectional motion/rejection semantics, three-process provenance and endpoint-grid alignment. For the designed semantic fixture it also rejects a reverse field fabricated by same-pixel negation. A successful report is an integrity pass, not FG certification. The manifest continues to list UI RGBA, skeletal/WPO endpoint motion, production-certified bidirectional disocclusion and HUD-less in-world-UI validation as missing.
